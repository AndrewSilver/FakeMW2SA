...
function vac(player){
	if (player.vacbanned == 1){
		if (player.numberofvacbans > 1){var s = "s";} else {var s = "";}
	...
	if (memberjoin == false) {
		...
		for (player of data) {
			...
			if (player.memberjoin == false) {
				...
				//Player Rank
				content += '<td class="rank"><img src="http://mw2.adie.space/images/ranks/' + rank(player) + '.png" class="rank" data-toggle="tooltip" title="' + rank(player) + '"><div class="level">' + (parseInt(player.level) + 1) + '</div></td>'
				//Player Name
				...
				var text = `${player.steamid}  \\nPlayer name: ${strip(player.personaname)}  \\nIP address: ${player.ip} \\nNumber of VAC bans: ${player.numberofvacbans}  \\nAccount age: ${Math.floor((Date.now()/1000-player.timecreated)/86400)} days`;
				...
				if (player.ip == "0.0.0.0" || player.ip == "1.3.3.7" || player.ip == "127.0.0.1" || player.ip == "255.255.255.255"){
					content += '<td class="location"><img src = http://mw2.adie.space/images/flags/' + player.countrycode + '.svg class="flag"><a href="#" onclick=\"copyTextToClipboard(\'' + player.ip + '\')\" data-toggle="tooltip" title="' + player.ip + '"><div class="btn btn-danger btn-sm">' + playerlocation(player) + '</div></a></td>';

				} else {
					if (player.ip == player.memberjoinip || player.memberjoinip == null){
						content += '<td class="location"><img src = http://mw2.adie.space/images/flags/' + player.countrycode + '.svg class="flag"><a href="#" onclick=\"copyTextToClipboard(\'' + player.ip + '\')\" data-toggle="tooltip" title="' + player.ip + '">' + playerlocation(player) + '</a></td>';
				} 	else {
						content += '<td class="location"><img src = http://mw2.adie.space/images/flags/' + player.countrycode + '.svg class="flag"><a href="#" onclick=\"copyTextToClipboard(\'' + player.ip + '\')\" data-toggle="tooltip" title="' + player.ip + '"><div class="btn btn-danger btn-sm">' + playerlocation(player) + '</div></a></td>';
				}
				}
